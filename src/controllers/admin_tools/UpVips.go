...
// UpVips 升级vip
func UpVips(c *gin.Context) {
	platform := request.GetPlatform(c)
	username := "" // 默认用户名称

	dateStart, dateEnd := func() (string, string) { // 开始时间/结束时间
		if createStr := c.DefaultQuery("created", ""); createStr != "" {
			dArr := strings.Split(createStr, " - ")
			return dArr[0], dArr[1]
		}
		currentTime := tools.NowTime()
		dateStart := tools.FirstDayOfMon(currentTime)
		dateEnd := tools.LastDayOfMon(currentTime)
	...
	total := 0 // 总计记录数量
	rows := models.GetShouldUpUsers(dateStart, dateEnd, platform, page, &total, username)

	viewFile := "admin_tools/up_vips.html"
	if request.IsAjax(c) {
		viewFile = "admin_tools/_up_vips.html"
	}
